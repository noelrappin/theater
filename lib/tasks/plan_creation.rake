  ...
  task create_plans: :environment do
    plans = [
      {remote_id: "orchestra_monthly", plan_name: "Orchestra Monthly",
       amount_cents: 10_000, interval: "monthly", tickets_allowed: 1,
       ticket_category: "Orchestra"},
      {remote_id: "balcony_monthly", plan_name: "Balcony Monthly",
       amount_cents: 60_000, interval: "monthly", tickets_allowed: 1,
       ticket_category: "Balcony"},
      {remote_id: "vip_monthly", plan_name: "VIP Monthly",
       amount_cents: 30_000, interval: "monthly", tickets_allowed: 1,
       ticket_category: "VIP"}
    ]
    Plan.transaction do
      plans.each { |plan_data| CreatesPlans(**plan_data).run }
    end
  end
