...
RSpec.describe Payment, :aggregate_failures, type: :model do
  ...
  describe "copy for refund" do
    ...
    let!(:payment_line_item) { PaymentLineItem.create(
      payment_id: payment.id, reference_id: ticket.id, price_cents: 1500,
      reference_type: "Ticket", refund_status: "no_refund") }
    ...
    it "creates a refund copy" do
      ...
      expect(line_item).to have_attributes(
        payment_id: refund_payment.id, reference_id: ticket.id,
        price_cents: -1500, reference_type: "Ticket",
